...
class UnetHook(nn.Module):
    ...
    def hook(self, model, sd_ldm, control_params, process, unet_engine=None, control_engine=None):
        self.model = model
        self.sd_ldm = sd_ldm
        ...
        def forward(self, x, timesteps=None, context=None, **kwargs):
            ...
            for param in outer.control_params:
                ...
                if outer.control_engine is None:
                    ...
                    engine_vram_req = outer.control_engine.engine.device_memory_size
                    tmp = torch.empty(engine_vram_req, dtype=torch.uint8, device="cuda")
                    outer.control_engine.context.device_memory = tmp.data_ptr()
                    cudaStream = torch.cuda.current_stream().cuda_stream
                    outer.control_engine.allocate_buffers(control_feed_dict)

                    controls = outer.control_engine.infer(control_feed_dict, cudaStream)
                    control = []
                    for idx in range(13):
                        control.append(controls[f"control_{idx}"])
                    nvtx.range_pop()
            ...
            if isinstance(total_controlnet_embedding[0], float):
                total_controlnet_embedding = [
                    torch.zeros([x.shape[0], 320, x.shape[2], x.shape[3]]).cuda(),
                    torch.zeros([x.shape[0], 320, x.shape[2], x.shape[3]]).cuda(),
                    torch.zeros([x.shape[0], 320, x.shape[2], x.shape[3]]).cuda(),
                    torch.zeros([x.shape[0], 320, x.shape[2] // 2, x.shape[3] // 2]).cuda(),
                    torch.zeros([x.shape[0], 640, x.shape[2] // 2, x.shape[3] // 2]).cuda(),
                    torch.zeros([x.shape[0], 640, x.shape[2] // 2, x.shape[3] // 2]).cuda(),
                    torch.zeros([x.shape[0], 640, x.shape[2] // 4, x.shape[3] // 4]).cuda(),
                    torch.zeros([x.shape[0], 1280, x.shape[2] // 4, x.shape[3] // 4]).cuda(),
                    torch.zeros([x.shape[0], 1280, x.shape[2] // 4, x.shape[3] // 4]).cuda(),
                    torch.zeros([x.shape[0], 1280, x.shape[2] // 8, x.shape[3] // 8]).cuda(),
                    torch.zeros([x.shape[0], 1280, x.shape[2] // 8, x.shape[3] // 8]).cuda(),
                    torch.zeros([x.shape[0], 1280, x.shape[2] // 8, x.shape[3] // 8]).cuda(),
                    torch.zeros([x.shape[0], 1280, x.shape[2] // 8, x.shape[3] // 8]).cuda()
                ]

            if outer.unet_engine is None:
                hs = []
                with th.no_grad():
                ...
                for i in range(13):
                    feed_dict[f"control_{i}"] = total_controlnet_embedding[i]

                nvtx.range_push("forward")
                engine_vram_req = outer.unet_engine.engine.device_memory_size
                tmp = torch.empty(engine_vram_req, dtype=torch.uint8, device="cuda")
                outer.unet_engine.context.device_memory = tmp.data_ptr()
                cudaStream = torch.cuda.current_stream().cuda_stream
                outer.unet_engine.allocate_buffers(feed_dict)

                h = outer.unet_engine.infer(feed_dict, cudaStream)["latent"]
                nvtx.range_pop()
